...
DECLARE
  ...
  v_wide integer := 60;
  v_k    integer := 60; -- RRF constant
BEGIN
  -- Better recall for HNSW
  PERFORM set_config('hnsw.ef_search', '100', true);

  -- Flexible FTS: websearch handles quotes / OR / -term; fallback to OR-of-terms.
  BEGIN
    v_tsquery := websearch_to_tsquery('spanish', COALESCE(p_question, ''));
    IF v_tsquery IS NULL OR v_tsquery::text = '' THEN
      v_tsquery := to_tsquery(
        'spanish',
        NULLIF(regexp_replace(trim(p_question), '\s+', ' | ', 'g'), '')
      );
    END IF;
