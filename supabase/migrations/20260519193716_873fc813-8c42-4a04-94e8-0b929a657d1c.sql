CREATE OR REPLACE FUNCTION public.rag_hybrid_search(p_question text, p_query_embedding vector, p_dominio text DEFAULT NULL::text, p_proyecto_id uuid DEFAULT NULL::uuid, p_limit integer DEFAULT 20, p_dominios text[] DEFAULT NULL::text[])
 RETURNS TABLE(id uuid, documento_id uuid, proyecto_id uuid, contenido text, dominio text, metadata jsonb, owner_id uuid, visibility text, fts_rank real, vec_distance real, hybrid_score real)
 LANGUAGE plpgsql
 STABLE
